...
def RegressionResult(runs = 100, kind='Linear'):
    ...
    for i in range(runs):
        ...
        X_train, X_test, y_train, y_test = train_test_split(\
                            scaler, y, test_size=0.25, random_state=42)

        if kind == 'Linear':
            # --- linear regression
            regr = LinearRegression(fit_intercept=False)
        elif kind == 'Lasso':
            regr = Lasso(fit_intercept=False, alpha = 5.)
        else:
...
runLasso = RegressionResult(kind='Lasso')
...
temp['R2_LassoRegression'] = runLasso['R2_Score']
